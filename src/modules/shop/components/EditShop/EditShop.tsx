...
export default function EditShop({ shopId, session }) {
  ...
  const handleNext = () => {
    setCurrentIndex((prevIndex) =>
      prevIndex === fileURLsPoster.length - 1 ? 0 : prevIndex + 1
    );
  };

  const handlePrev = () => {
    setCurrentIndex((prevIndex) =>
      prevIndex === 0 ? fileURLsPoster.length - 1 : prevIndex - 1
    );
  };
  ...
  return (
    <>
      <div className="flex flex-col">
        <div className="rounded-b-3xl drop-shadow-md h-56 overflow-hidden">
          {fileURLsPoster.length > 0 ? (
            <>
              <div className="w-full h-full">
                <img
                  src={fileURLsPoster[currentIndex]}
                  alt={`Slide ${currentIndex + 1}`}
                  className="w-full h-full object-cover"
                />
              </div>
              {fileURLsPoster.length > 1 && (
                <>
                  <button
                    onClick={handlePrev}
                    className="absolute top-1/2 left-4 -translate-y-1/2 bg-black/70 text-white px-2 py-1 rounded-full hover:bg-black transition"
                  >
                    ←
                  </button>
                  <button
                    onClick={handleNext}
                    className="absolute top-1/2 right-4 -translate-y-1/2 bg-black/70 text-white px-2 py-1 rounded-full hover:bg-black transition"
                  >
                    →
                  </button>
                </>
